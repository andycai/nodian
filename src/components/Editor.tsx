...
//   }, [openFiles]);
  ...
  const markdownComponents = {
    h1: ({node, ...props}) => <h1 style={{fontSize: '2.5em', borderBottom: '1px solid #d0d7de', paddingBottom: '.3em', marginTop: '24px', marginBottom: '16px'}} {...props}/>,
    h2: ({node, ...props}) => <h2 style={{fontSize: '2em', borderBottom: '1px solid #d0d7de', paddingBottom: '.3em', marginTop: '24px', marginBottom: '16px'}} {...props}/>,
    ...
    blockquote: ({node, ...props}) => <blockquote style={{padding: '0 1em', color: '#57606a', borderLeft: '0.25em solid #d0d7de', marginBottom: '16px'}} {...props}/>,
    code: ({node, inline, ...props}) => 
      inline 
        ? <code style={{background: 'rgba(175,184,193,0.2)', padding: '.2em .4em', borderRadius: '6px', fontSize: '85%', fontFamily: 'ui-monospace,SFMono-Regular,SF Mono,Menlo,Consolas,Liberation Mono,monospace'}} {...props}/>
        : <pre style={{background: '#f6f8fa', padding: '16px', overflow: 'auto', fontSize: '85%', lineHeight: '1.45', borderRadius: '6px', marginBottom: '16px'}}><code style={{fontFamily: 'ui-monospace,SFMono-Regular,SF Mono,Menlo,Consolas,Liberation Mono,monospace'}} {...props}/></pre>,
    img: ({node, ...props}) => <img style={{maxWidth: '100%', boxSizing: 'content-box'}} {...props} />
  };

  return (
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-gray-900">
      <div className="flex border-b border-gray-200 dark:border-gray-700 overflow-x-auto bg-gray-100 dark:bg-gray-800">
        {openFiles.map(file => (
          <div
            key={file}
            className={`px-4 py-2 cursor-pointer flex items-center whitespace-nowrap ${
              file === selectedFile ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400' : 'hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300'
            }`}
      ...
      <div className="flex-1 flex overflow-hidden">
        {selectedFile && (
          <>
            <textarea
              className="flex-1 p-4 resize-none outline-none bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 overflow-auto"
            ...
            />
            <div
              className="flex-1 p-4 overflow-auto bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200"
      ...
      <div className="flex justify-between p-2 border-t border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
        <button
